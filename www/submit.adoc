...
## Difference to the General Submission Rules
...
This document lists the diffences for the MLCommons Science Group Submissions. The differences are summarized by
...
---
> Note: The original directory struture rule Section 13 will be removed from the document
>
...
In this section we document the directory structure for submissions. We introduce the following variables denoted by `{ }` around the Variable name. The backets `[ ]` are used to donate a list
...
3. `config.yaml` ::= A configuration file that contains all hyperparameters and
   ...
   The configuration file should include enough deatails to replicate the
   experiment with locations to the program and the data. If the data does not fit in a GitHub repository it can be placed in a public acessible data store. Its location needs to be specified as endpoint in the yaml file, with a commandline example on how to retrieve it. As multiple files could be needed a list of commands can be specified.
...
6. Log File requirements.
   ...
   1. The log file must have am Oraganization Records in Mllog entry format. This includes mllog entries for `POINT_IN_TIME` with the values
